    ...
    def get_formatted_response(cls, probe_id, created, response):
        ...
        if response.abuf.edns0:
            edns = (
                "\n  ;; OPT PSEUDOSECTION:\n  ; EDNS: version: {0}, "
                "flags:; udp: {1}\n".format(
                    response.abuf.edns0.version, response.abuf.edns0.udp_size
                )
            )
        ...
        return cls._colourise_by_response(
            ...
            cls.render_template(
                ...
                question=sanitise(
                    cls.get_section("question", response.abuf.questions),
                    strip_newlines=False,
                ),
                answers=sanitise(
                    cls.get_section("answer", response.abuf.answers),
                    strip_newlines=False,
                ),
                authorities=sanitise(
                    cls.get_section("authority", response.abuf.authorities),
                    strip_newlines=False,
                ),
                additionals=sanitise(
                    cls.get_section("additional", response.abuf.additionals),
                    strip_newlines=False,
                ),
